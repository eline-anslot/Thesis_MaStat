...
sim <- function(p_short, months_short, n_sim, p_LTE_1, design_test) {
  p_LTE_0 <- .25
  alpha <- .05
  power_start <- .80 
  ...
  max_rec <- end_n149[end_n149$lambda == l, "mean"]
  sd_rec <- end_n149[end_n149$lambda == l, "sd"]
  ...
  for (i in seq_len(nrow(design_test))){
    ...
    for (j in 1:n_sim) {
      ...
      sim_data[indexation, "p_short"]        <- rep(p_short,
                                                    time = n_methods)
      sim_data[indexation, "time_short"]     <- months_short
  ...
  directory_name <- paste("./data_equi_nextra_timeshort_n_", design_test$n, "_", n_sim, sep="")
  dir.create(directory_name, showWarnings = FALSE)
  write.csv(sim_data, paste(directory_name, "/sim_data", p_short, "_", months_short, ".csv", sep=""))
}